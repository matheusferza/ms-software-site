        ...
        <BaseLayout {...props}>
            <article className="px-4 py-14 lg:py-20">
                <div className="detail-article">
                    <header className="max-w-5xl mx-auto mb-10 sm:mb-14">
                        {client && <div className="detail-meta md:mb-6">{client}</div>}
                        <div className="flex flex-col gap-6 md:flex-row md:justify-between">
                            <time className="detail-meta md:order-last" dateTime={dateTimeAttr}>
                                {formattedDate}
                            </time>
                    ...
                    {description && (
                        <div className="max-w-3xl mx-auto mb-10 text-lg text-white/80 sm:text-xl sm:mb-14">
                            {description}
                        </div>
                    )}
                    {media && (
                        <figure className="detail-media max-w-5xl mx-auto mb-10 sm:mb-14">
            ...
            {(prevProject || nextProject) && (
                <nav className="px-4 mt-12 mb-20">
                    <div className="grid max-w-5xl mx-auto gap-x-6 gap-y-6 sm:grid-cols-2 lg:gap-x-8">
...
function ProjectNavItem({ project, className }) {
    return (
        <Annotated content={project}>
            <Link className={classNames('detail-nav-card group flex flex-col gap-6 items-start', className)} href={project}>
                {project.featuredImage && (
                    <div className="feed-card-image w-full overflow-hidden aspect-3/2">
                        <ImageBlock {...project.featuredImage} className="object-cover w-full h-full" />
                    </div>
                )}
                <div className="premium-card-meta">Explorar projeto</div>
                <span className="premium-card-title">{project.title}</span>
            </Link>
        </Annotated>
    );
}
